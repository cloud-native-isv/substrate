...
- **平面一（Tier-1，worker pod 面）**：**Kubernetes + Substrate + Containerd + runc/rund** 构建基于 Kubernetes 生态的 agent sandbox **worker pod**（sandbox class `wasm` 的 kata 形态）。特性要求：**复用 Kubernetes 生态**、**低频生命周期 + 高稳定性**、提供**休眠唤醒（Suspend/ResumeActor）**等 agent sandbox 基础特性。
...
| 旧术语 | 归一后 | 上游锚点 |
|---|---|---|
| supervisor sandbox | **worker pod**（sandbox class `wasm` 的 kata 形态） | Worker/WorkerPod：温沙箱 pod，host RUNNING actor |
...
| supervisor（新 enum 值，早期版提议） | **废除：不新增 enum**——两层形态 = `wasm` class + `WorkerPool.runtimeClassName: rund` | SandboxClass enum 保持 gvisor/microvm/wasm |
...
```
Tier-1 控制面(ateapi/atecontroller)   Tier-2 请求面(e2bgw+自研管控)   Worker Pod(wasm class kata 形态)   Context(wasm 执行)
        |                                        |                          |                          |
 CreateActor(session)                            |                          |                          |
...
- **Ateom 协议**：wasm class（kata 形态）的 `RunWorkload` = 一次请求执行（携 capability manifest，ateom 内 spawn/destroy context）；`Checkpoint/RestoreWorkload` 作用域 = actor（wasm 状态 + workspace）；
...
- 新 actor 默认两层模型；迁移完成条件 = 所有 ActorTemplate 切到 `sandboxClass: wasm` + 池 `runtimeClassName: rund` 形态且 legacy 池缩容至 0；
