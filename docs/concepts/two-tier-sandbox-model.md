...
- **SandboxClass**：**不新增 enum**——两层结构 = 既有 `wasm` class；**pod runtime 正交旋钮** `WorkerPool.runtimeClassName`（xuanji 扩展字段）选 `runc`（基线默认）或 `rund`（kata 可选强隔离），两层不依赖 kata；**runc 基线以「wasm 隔离可信」为前提**（host function/wasmtime 缺陷击穿 wasm 时 runc 不足、须 rund 兜底；判据 = wasm 可信度 × 节点租户模型，成熟度早期默认 rund）；legacy 单层 wasm-pool 是旧结构（§5），与 runtime 无关；gvisor/microvm 不变；
