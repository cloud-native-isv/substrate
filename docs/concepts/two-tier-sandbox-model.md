...
隔离分层（纵深防御栈）：**主要边界 = wasm 线性内存**（context ↔ 同 actor 其他 context，并隔离租户代码——真正承载租户隔离的层）；**次要边界 = pod runtime**（按旋钮：runc 容器级 / rund kata VM 内核级，隔离 worker pod ↔ 宿主/邻 pod，rund 用于跨租户共享节点）。**外层须兜住内层失效**：缺陷 host function / wasmtime 漏洞 → 租户代码逃逸出 wasm 落入控制面容器原生进程 → runc（共享宿主内核）不足兜底、rund（独立 guest kernel）把逃逸关在 VM 内。**故 runc 充分性以「wasm 隔离可信」为前提**（host function 硬化/fuzzing/审计前倾向 rund；判据 = wasm 可信度 × 节点租户 × 敏感度 × 密度/成本）。runc/rund **各具独立价值**（runc 资源利用率高/场景广，rund 安全性高），是每租户池策略旋钮、混合机群为预期形态。**actor 不以容器隔离**——容器固定（控制面/业务面双容器，同属平台信任域），actor 间隔离在控制面容器内以 wasm 承载。
...
- **SandboxClass**：**不新增 enum**——两层结构 = 既有 `wasm` class；**pod runtime 正交旋钮** `WorkerPool.runtimeClassName`（xuanji 扩展字段）选 `runc` 或 `rund`（kata），**二者各具独立价值**（runc 资源利用率高/场景广，rund 安全性高），是**每租户池策略**（混合机群为预期形态），两层不依赖 kata；**runc 充分性以「wasm 隔离可信」为前提**（host function/wasmtime 缺陷击穿 wasm 时 runc 不足、须 rund 兜底；判据 = wasm 可信度 × 节点租户 × 敏感度 × 密度/成本）；legacy 单层 wasm-pool 是旧结构（§5），与 runtime 无关；gvisor/microvm 不变；
