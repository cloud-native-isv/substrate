...
| `internal/actorlog/logger.go` | 新增 `TrustDomain` 类型 + 5 个信任域常量（I/P/S/Agent/MCP）、`CrossDomainAudit` 结构、`reservedAuditKeys` 与 `EmitCrossDomainAudit()` 方法（统一跨域审计 schema，字段名 `event`/`src_domain`/`dst_domain`/`allowed`/`reason` 刻意对齐 sandbox 仓 S13 `src/runtime/src/audit.rs` 的 `mcp.call` 事件；保留键不被 `Fields` 覆盖）；纯追加，不改既有 `EmitLifecycleLog`/`WrapContainerLogs` | F12 跨域审计统一（全景图 §5「每次跨域操作皆白名单+审计」P 侧落地；split doc §8.3 F12；与 sandbox S13 构成跨仓统一审计平面；deny-by-default 裁决审计见 ADR 0003 D1） |
...
| `docs/decisions/0002-two-tier-supervisor-worker-sandbox.md`、`docs/concepts/two-tier-sandbox-model.md`、`manifests/xuanji/two-tier-example.yaml`、`docs/tasks/substrate-upstream-regression.md` |（2026-09-17 随分支归一提交，原 substrate-preflight worktree 未提交产物；**2026-09-17 术语归一**：概念全面对齐上游 substrate 词汇——supervisor sandbox→worker pod、session→actor、worker sandbox→actor 执行体 wasm sandbox + 请求级 workload 执行 context、suspend 归一为 actor 级 checkpoint；**2026-09-17 去 supervisor 语义**：不新增 SandboxClass enum，两层形态由既有 `wasm` class + `WorkerPool.runtimeClassName: rund`（xuanji 扩展字段）承载，herder 即 ateom-wasmd 演进版）两层沙箱模型 ADR 0002（Tier-1 worker pod 面 K8s+Substrate+Containerd+runc/rund / Tier-2 actor 工作负载面 e2b+自研管控+wasm workload）+ 配套概念文档与目标 schema 示例清单 + substrate 回归开源评估（fork 改动 A/B/C 分类与向 sandbox 仓迁移方案） |
| `internal/actorlog/cross_domain_audit_test.go` | F12 跨域审计单测（2 例）：`TestEmitCrossDomainAuditUnifiedSchema`（断言 event/src_domain=P/dst_domain=S/allowed/自定义 field，保留键 `event`/`src_domain` 不被 `Fields` 的 HACK-attempt 覆盖，allowed=true 时无 reason，actor 标签）+ `TestEmitCrossDomainAuditDenyCarriesReason`（allowed=false 带 reason，S→MCP 跨域）；与 logger.go 的 `EmitCrossDomainAudit` 配套 |
| `internal/mcppool/{pool.go,pool_test.go}` | F10-core：P 层 MCP 共享池**路由决策核**（全景图 §3.4/§3.5、ADR 0007 D4、自洽条件 C2「共享池编排/路由层 = P 可信+硬化+自身被审计」）。`ExecutionUnit`（per-tenant，F11 缝；OS 多用户非跨租户边界故单元按租户隔离=C1）+ `PoolConfig`（`AllowedTenants` 跨-S allowlist，空=拒绝全部 fail-closed D9）+ `Router.Route(ref,uid,tmpl*,tool)→Decision` 两道 deny-by-default 闸（① 租户 allowlist=R3 confused-deputy 防御；② per-tenant 单元解析+工具 allowlist=D7），每次裁决经注入的 `AuditEmitter`（`*actorlog.ActorLogger` 满足）发一条 F12 跨域审计（event=`mcp.pool.route`，src_domain=S/dst_domain=MCP），与 sandbox S13 `mcp.call` 归同一审计平面。**纯库、零 upstream 改动面、无 proto/CRD 变更**（执行单元 provisioning=F11、ateom→池 mTLS 传输=S10 缝，均不在本核内）；`Decision.Unit` 返回副本防配置别名污染。7 单测覆盖两闸拒绝/允许/per-tenant 不越界(R3)/fail-closed/nil emitter 不 panic/单元副本 |
