...
| `docs/decisions/0002-two-tier-supervisor-worker-sandbox.md`、`docs/concepts/two-tier-sandbox-model.md`、`manifests/xuanji/two-tier-example.yaml`、`docs/tasks/substrate-upstream-regression.md` |（2026-09-17 随分支归一提交，原 substrate-preflight worktree 未提交产物；**2026-09-17 术语归一**：概念全面对齐上游 substrate 词汇——supervisor sandbox→worker pod（sandbox class supervisor）、session→actor、worker sandbox→actor 执行体 wasm sandbox + 请求级 workload 执行 context、suspend 归一为 actor 级 checkpoint）两层沙箱模型 ADR 0002（Tier-1 worker pod 面 K8s+Substrate+Containerd+runc/rund / Tier-2 actor 工作负载面 e2b+自研管控+wasm workload）+ 配套概念文档与目标 schema 示例清单 + substrate 回归开源评估（fork 改动 A/B/C 分类与向 sandbox 仓迁移方案） |
