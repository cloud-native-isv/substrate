...
Reference patterns for editing `word/document.xml` and related files inside an unpacked `.docx`.
...
After running `comment.py` (see [docx-editing.md](./docx-editing.md)), add markers to document.xml. For replies, use `--parent` flag and nest markers inside the parent's.
