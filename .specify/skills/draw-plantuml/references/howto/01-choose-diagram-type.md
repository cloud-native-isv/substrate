...
当任务/需求清单**点名要求若干类图**（如"5 类图：组件 / 部署 / 时序 / 状态机 / 资源模型"）时，在 Step 2 规划完图集后，**逐项核对清单中的每类图都有对应的独立图表**：

- 每类被点名的图类型必须有一张**独立的、语义完整的图表**交付；**以 note/文字摘要形式顺带出现在别的图里不算完成该类图**（如"状态机只在时序图底部一行 note"不构成状态机图交付物）
- 被点名的图类型无专门操作指南时，用相近类型承载并明示：分层资源模型 → 包图/组件图（[06-package-diagram.md](06-package-diagram.md) 双层资源模型）、状态流转 → 状态机图（[09-state-machine-diagram.md](09-state-machine-diagram.md)）、部署拓扑 → 部署图（[04-deployment-diagram.md](04-deployment-diagram.md)）
- 核对结果写进图集规划：**哪类需求 → 哪张图**；缺哪类 → 规划阶段就补哪张；交付前按清单逐项勾销，避免交付时才发现缺图
