...
- **需求点名状态机时，必须交付独立状态机图**：当任务/需求清单明确列出「XX 状态机」为交付物时，产出**独立的 PlantUML stateDiagram**（`state` + `-->` 转换，覆盖初始/终止态与全部关键转换），**不能**只在时序图/组件图底部用 note 摘要状态流转敷衍——note 摘要不构成状态机图交付物（见 [01-choose-diagram-type.md 需求清单覆盖检查](01-choose-diagram-type.md)）；状态机语义越复杂，越值得独立成图
