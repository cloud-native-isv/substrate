...
Spec Kit 全部改进机制共同服务的目标的**单一事实源**：feedback 机制（`workflow/feedback-step.md`）、证据层（`workflow/evidence-step.md` + `collect-evidence`）、Self-Improvement（概念见 `definitions/self-improvement-definitions.md`）、`create-*` / `improve-*` 技能族、memory / history / review，各自解决不同环节的问题，但都指向同一件事——**让项目成为对 AI agent 更好的执行环境（a better harness）**。各流程只引用本文件，不复制定义；修订本文件即修订全部消费者的目标表述。
...
| **Learning Capture**（经验沉淀） | 下一个任务是否从本次受益？ | memory（session / knowledge）、history、**feedback 机制**、Self-Improvement workflow、`improve-*` 技能族 |
...
- **不把 Self-Improvement 变成自治运行时**：Self-Improvement 只组合现有 feedback/evidence/improve/validation 资产；其身份边界和执行路径分别见 `definitions/self-improvement-definitions.md` 与 `workflow/self-improvement-workflow.md`。
...
| `definitions/self-improvement-definitions.md` | 将 Better Harness 定位为 Self-Improvement 的目标层，而非触发器或执行引擎 |
| `workflow/self-improvement-workflow.md` | 每个干预在 SI-3 选择本表维度，跨维度问题升级到系统级辅助评审 |
| `create-*` / `improve-*` skills | 创建时安装主体契约；改进时声明目标维度并执行证据驱动干预 |
| `/speckit.review` / `/better-harness` | 系统级辅助评审按五维组织发现 |
