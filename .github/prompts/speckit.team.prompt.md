...
1. **识别(确定性,零语义猜测)**:先运行 `python3 .specify/scripts/python/goal-utils.py list --json` 取得 archive slug 全集;入参 token 与某 slug **精确匹配**(或为指向其 `goal.md` 的路径)→ 向用户确认"基于已定义 goal `<slug>` 创建"后进入分支。匹配判定 MUST 由引擎枚举结果驱动,MUST NOT 由 agent 凭记忆或语义猜测断言;近似不匹配(大小写/连字符差异)**不构成命中**。无命中 → 走既有自由文本流程,行为与引入前一致(零回归)。
2. **加载与两类拒绝**:进入分支后经 `parse_goal`(`.specify/scripts/python/goal-utils.py`)读取 objective、criteria(含 `None provided.` 缺失态)、status、targets、history,并复述给用户确认。
